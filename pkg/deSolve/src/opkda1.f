...
C**End
      ...
      INTEGER IOWNS, IXDUM1, IXDUM2, IXDUM3, IXDUM4, IXDUM5, IXDUM6,
     ...
     1   CCMAX, EL0, H, HMIN, HMXI, HU, RC, TN, UROUND,
     2   IXDUM1, IXDUM2, IXDUM3, IXDUM4, IXDUM5, IXDUM6, IOWNS(6),
...
CKS: added rpar,ipar
     ...
     2   LYH, LEWT, LACOR, LSAVF, LWM, LIWM, METH, MITER,
     3   MAXORD, MAXCOR, MSBP, MXNCF, N, NQ, NST, NFE, NJE, NQU
      INTEGER IOWND21, IOWND22, IOWND23, ICOUNT, IRFLAG, JTYP, 
     1   MUSED, MXORDN, MXORDS
      DOUBLE PRECISION CONIT, CRATE, EL, ELCO, HOLD, RMAX, TESCO,
     2   CCMAX, EL0, H, HMIN, HMXI, HU, RC, TN, UROUND
     ...
     1   PDNORM, IOWND21, IOWND22, IOWND23,
     2   ICOUNT, IRFLAG, JTYP, MUSED, MXORDN, MXORDS
      INTEGER I, I1, IREDO, IRET, J, JB, M, NCF, NEWQ
...
CKS: added rpar,ipar
     ...
     2   LYH, LEWT, LACOR, LSAVF, LWM, LIWM, METH, MITER,
     3   MAXORD, MAXCOR, MSBP, MXNCF, N, NQ, NST, NFE, NJE, NQU
      INTEGER IOWND21, IOWND22, IOWND23, IOWNS21, IOWNS22, JTYP, 
     1   MUSED, MXORDN, MXORDS
      DOUBLE PRECISION ROWNS,
     1   CCMAX, EL0, H, HMIN, HMXI, HU, RC, TN, UROUND
      DOUBLE PRECISION ROWND2, RCM1, RCM2, ROWNS21, ROWNS22, 
     1   ROWNS23, PDNORM
      COMMON /DLS001/ ROWNS(209),
     1   CCMAX, EL0, H, HMIN, HMXI, HU, RC, TN, UROUND,
     2   IOWND(6), IOWNS(6),
     ...
     4   LYH, LEWT, LACOR, LSAVF, LWM, LIWM, METH, MITER,
     5   MAXORD, MAXCOR, MSBP, MXNCF, N, NQ, NST, NFE, NJE, NQU
      COMMON /DLSA01/ ROWND2, RCM1(12), RCM2(5), 
     1   ROWNS21, ROWNS22, ROWNS23, PDNORM,
     1   IOWND21, IOWND22, IOWND23, IOWNS21, IOWNS22, JTYP, 
     2   MUSED, MXORDN, MXORDS
      INTEGER I, I1, I2, IER, II, J, J1, JJ, LENP,
     1   MBA, MBAND, MEB1, MEBAND, ML, ML3, MU, NP1
...
*DECK DRCHEK
     ...
     1   rpar, ipar)
      EXTERNAL G
      INTEGER JOB, NEQ, NYH, JROOT, IRT, ipar(*)
      DOUBLE PRECISION Y, YH, G0, G1, GX, rpar(*)
...
C Evaluate g at initial T, and check for zero values. ------------------
 100  CONTINUE
      T0 = TN
      CALL G (NEQ, T0, Y, NGC, G0, rpar, ipar)
      NGE = 1
      ZROOT = .FALSE.
      DO 110 I = 1,NGC
...
C g has a zero at T0.  Look at g at T + (small increment). -------------
 ...
 240  CALL G (NEQ, T0, Y, NGC, G0, rpar, ipar)
      NGE = NGE + 1
      ZROOT = .FALSE.
      DO 250 I = 1,NGC
...
C Set T1 to TN or TOUTC, whichever comes first, and get g at T1. -------
 ...
 310  T1 = TN
      DO 320 I = 1,N
        Y(I) = YH(I,1)
 320  CONTINUE
 330  CALL G (NEQ, T1, Y, NGC, G1, rpar, ipar)
      NGE = NGE + 1
C Call DROOTS to search for root in interval from T0 to T1. ------------
 ...
 350  CONTINUE
      ...
      CALL G (NEQ, X, Y, NGC, GX, rpar, ipar)
      NGE = NGE + 1
      GO TO 350
 360  T0 = X
...
C-----------------------------------------------------------------------
 ...
 360  CONTINUE
      CALL DSOLPK (NEQ, Y, SAVF, SAVX, EWT, WM, IWM, F,PSOL,rpar,ipar)
      IF (IERSL .LT. 0) GO TO 430
      IF (IERSL .GT. 0) GO TO 410
...
C
     ...
     1   WM(LOCWP), IWM(LOCIWP), IER,rpar,ipar)
      NJE = NJE + 1
      IF (IER .EQ. 0) RETURN
      IERPJ = 1
      RETURN
...
*DECK DSOLPK
     ...
     1 rpar,ipar)
      EXTERNAL F, PSOL
      INTEGER NEQ, IWM     ,ipar(*)
      DOUBLE PRECISION Y, SAVF, X, EWT, WM,rpar(*)
      DIMENSION NEQ(*), Y(*), SAVF(*), X(*), EWT(*), WM(*), IWM(*)
      INTEGER IXDUM1, IXDUM2, IXDUM3, IXDUM4, IXDUM5, IXDUM6, IOWNS,
     1   ICF, IERPJ, IERSL, JCUR, JSTART, KFLAG, L,
     2   LYH, LEWT, LACOR, LSAVF, LWM, LIWM, METH, MITER,
     3   MAXORD, MAXCOR, MSBP, MXNCF, N, NQ, NST, NFE, NJE, NQU
      INTEGER JPRE, JACFLG, LOCWP, LOCIWP, LSAVX, KMP, MAXL, MNEWT,
...
C
      ...
      ELSE IF (MITER .EQ. 4) THEN
        GOTO 400
      ELSE IF (MITER .LE. 9) THEN
        GOTO 900
      ENDIF
C karline: changed from
C    GO TO (100, 200, 300, 400, 900, 900, 900, 900, 900), MITER
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
 100  CONTINUE
     ...
     2   LIOM, WM(LOCWP), IWM(LOCIWP), WM(LWK), IFLAG, rpar,ipar)
      NNI = NNI + 1
      NLI = NLI + LIOM
      NPS = NPS + NPSL
...
C-----------------------------------------------------------------------
 300  CONTINUE
     ...
     2          LPCG, WM(LOCWP), IWM(LOCIWP),WM(LWK),IFLAG,rpar, ipar)
      NNI = NNI + 1
      NLI = NLI + LPCG
      NPS = NPS + NPSL
      IF (IFLAG .NE. 0) NCFL = NCFL + 1
...
*DECK DSPIOM
     ...
     2            LIOM, WP, IWP, WK, IFLAG,rpar,ipar)
      EXTERNAL F, PSOL
      INTEGER NEQ,N,MAXL,KMP,JPRE,MNEWT,NPSL,IPVT,LIOM,IWP,IFLAG,ipar(*)
      DOUBLE PRECISION TN,Y,SAVF,B,WGHT,DELTA,HL0,X,V,HES,WP,WK,rpar(*)
...
C-----------------------------------------------------------------------
     ...
     1        WK, WP, IWP, HL0, JPRE, IER, NPSL, rpar,ipar)
        IF (IER .NE. 0) GO TO 300
...
*DECK DATV
     ...
     1                WP, IWP, HL0, JPRE, IER, NPSL,rpar,ipar)
      EXTERNAL F, PSOL
      INTEGER NEQ, IWP, JPRE, IER, NPSL ,ipar(*)
      DOUBLE PRECISION Y, SAVF, V, WGHT, FTEM, Z, VTEM, WP, HL0,rpar(*)
...
*DECK DSPIGMR
     ...
     2  LGMR, WP, IWP, WK, DL, IFLAG,rpar,ipar)
      EXTERNAL F, PSOL
      integer ipar(*)
      double precision rpar(*)
...
*DECK DPCG
     ...
     2 rpar,ipar)
      EXTERNAL F, PSOL
      INTEGER NEQ, N, MAXL, JPRE, MNEWT, NPSL, LPCG, IWP, IFLAG,ipar(*)
      DOUBLE PRECISION TN,Y,SAVF,R,WGHT,DELTA,HL0,X,P,W,Z,WP,WK,rpar(*)
...
C-----------------------------------------------------------------------
      CALL DATP (NEQ, Y, SAVF, P, WGHT, HL0, WK, F, W, rpar,ipar)
C
      PTW = DDOT (N, P, 1, W, 1)
...
C----------------------- End of Subroutine DPCGS -----------------------
      END
*DECK DATP
      SUBROUTINE DATP (NEQ, Y, SAVF, P, WGHT, HL0, WK, F, W,rpar,ipar)
      EXTERNAL F
      INTEGER NEQ, ipar(*)
      DOUBLE PRECISION Y, SAVF, P, WGHT, HL0, WK, W, rpar(*)
...
*DECK DLHIN
     ...
     1   EWT, ITOL, ATOL, Y, TEMP, H0, NITER, IER, rpar,ipar)
      EXTERNAL F
...
C Estimate the second derivative as a difference quotient in f. --------
 ...
 60   CONTINUE
CKS
      CALL F (NEQ, T1, Y, TEMP, rpar, ipar)
      DO 70 I = 1,N

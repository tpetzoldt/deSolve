...
\Abstract{
  ...
  The implementation includes stiff integration routines
  based on the ODEPACK Fortan codes \citep{Hindmarsh83}.

  It also include runge-kutta solvers and
  the euler method \citep{Press92}

  Here we outline how to implement differential equations as \R-functions.
...
@
  ...
  0.01 daily intervals. R's function seq() creates the time sequence.
...
\code{rk4} or \code{rk}     & integrates ODEs, using runge-kutta methods                                           \\ \hline
\code{euler}                & integrates ODEs, using euler method  \\ \hline
\hline
\end{tabular}
\end{table*}

...
C-----------------------------------------------------------------------
     ...
     2   IWORK(LIWM), F, JAC, PSOL, rpar, ipar)
      KGO = 1 - KFLAG
      GO TO (300, 530, 540, 550), KGO
C-----------------------------------------------------------------------
C Block F.
...
C
C A. First, provide a subroutine of the form:
C               SUBROUTINE RES (NEQ, T, Y, S, R, IRES, rpar,ipar)
C               DOUBLE PRECISION T, Y(*), S(*), R(*),rpar(*)
...
C               SUBROUTINE JAC (NEQ, T, Y, S, ML, MU, P, NROWP,rpar,ipar)
...
C     END
C
C     SUBROUTINE RESID(NEQ, T, Y, S, R, IRES,rpar,ipar)
C     DOUBLE PRECISION T, Y, S, R,rpar(*)
...
C DLSODI must compute initial dy/dt (LYD0 points to YH(*,2)). ----------
     ...
     1                ML, MU, RWORK(LP), IWORK(21), IER ,rpar,ipar)
         NFE = NFE + 1
...
C-----------------------------------------------------------------------
     ...
     2   IWORK(LIWM), RES, ADDA, JAC, DPREPJI, DSOLSY, rpar, ipar )
      KGO = 1 - KFLAG
...
C Compute residual if relevant. ----------------------------------------
 ...
 585     Y(I) = RWORK(I+LYH-1)
      IRES = 1
      CALL RES (NEQ, TN, Y, RWORK(LSAVF), YDOTI, IRES , rpar,ipar)
      NFE = NFE + 1
...
C DLSOIBT must compute initial dy/dt (LYD0 points to YH(*,2)). ---------
     ...
     1               MB, NB, RWORK(LP), IWORK(21), IER , rpar,ipar)
         NFE = NFE + 1
         IF (IER .LT. 0) GO TO 560
         IF (IER .GT. 0) GO TO 565
         DO 115  I = 1,N
...
C-----------------------------------------------------------------------
      CALL DSTODI (NEQ, Y, RWORK(LYH), NYH, RWORK(LYH), RWORK(LEWT),
     1   YDOTI, RWORK(LSAVF), RWORK(LACOR), RWORK(LWM),
     2   IWORK(LIWM), RES, ADDA, JAC, DPJIBT, DSLSBT , rpar, ipar)
      KGO = 1 - KFLAG
      GO TO (300, 530, 540, 400, 550), KGO
C
C KGO = 1:success; 2:error test failure; 3:convergence failure;
...
C DIPREPI and DPREPI do sparse matrix preprocessing. -------------------
     ...
     1   IWORK(LIC), IWORK(LJC), IPFLAG, RES, JAC, ADDA, rpar, ipar)
      LENRW = LWM - 1 + LENWK + LREST
      IWORK(17) = LENRW
      IF (IPFLAG .NE. -1) IWORK(23) = IPIAN
      IF (IPFLAG .NE. -1) IWORK(24) = IPJAN
...
C Compute initial dy/dt, if necessary, and load it into YH.-------------
     ...
     1              RWORK(LYD0), IER, RES, ADDA, rpar,ipar)
      NFE = NFE + 1
      IGO = IER + 1
...
C-----------------------------------------------------------------------
      CALL DSTODI (NEQ, Y, RWORK(LYH), NYH, RWORK(LYH), RWORK(LEWT),
     1   YDOTI, RWORK(LSAVF), RWORK(LACOR), RWORK(LWM),
     2   RWORK(LWM), RES, ADDA, JAC, DPRJIS, DSOLSS, rpar, ipar )
      KGO = 1 - KFLAG

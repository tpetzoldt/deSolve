...
\end{verbatim}
During compilation, we need to also compile the file dedeUtils.c. Assuming that the
above fortran code is in file \code{dede_lvF.f}, which is found in the working directory that 
...
We now give the same example in C-code (you will find this in directory \code{inst/doc/dynload-dede/dede_lv.c}).
\begin{verbatim}
#include <R.h>
#include <Rinternals.h>
#include <Rdefines.h>

...
(\code{rk..f}, \code{ode45}), Dormand and Prince \citep{Dormand1980,Dormand1981} 
(\code{rk..dp.}), Bogacki and Shampine \citep{Bogacki1989} 
((\code{rk23bs}), \code{ode23}) and Cash and Karp \citep{Cash1990} 
(\code{rk45ck}), where \code{ode23} and \code{ode45} are aliases for the popular methods \code{rk23bs} resp \code{rk45dp7}.
...
It is also possible to modify the parameters of a method (be very careful with this) 
or define and use a new Runge-Kutta method:

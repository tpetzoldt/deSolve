...
One of the prominent features of \deSolve is that it allows specifying
...
development, produce highly readible code and access to \R s
...
\end{verbatim}
where t is the current time point in the integration, y is the current
estimate of the variables in the ODE system, and parms is a vector or
list containing the parameter values. ... (optional) are any other
arguments passed to the function.  The return value of \emph{func}
should be a list, whose first element is a vector containing the
derivatives of y with respect to time, and whose second element
...
compiled code compared to \R-code, because it has to comply with the
...
\end{verbatim}
where \emph{*ml} and \emph{*mu} are the number of non-zero bands below
...
selected. \emph{*nrow} contains the number of rows of the Jacobian.
Only for full Jacobian matrices, this is equal to \emph{*neq}. In case
the jacobian is banded, \emph{*nrowpd} will be equal to
\emph{*mu+*ml+1}.
\footnote{readers familiar with the implementation of the FORTRAN code
  ...
  \emph{mu} empty rows. As we have taken the philosophy to make the
  model specification independent of the integrator that will be used,
  the facility to use \emph{vode} with a Jacobian specified in
...
\label{sec:parms}
All of the solvers in \deSolve take an argument \emph{parms} which may
be an arbitrary R object.  In models defined in \R code, this argument
...
The problem is that data passed to, say, \emph{ode} in
the argument \emph{parms} is not visible by default to the routines
that define the model.  This is handled by a user-written
initialization function, for example \emph{initmod} in the \C and \FOR
examples from sections \ref{sec:Cexamp} and \ref{sec:forexamp}.  In
\C, the initialization routine is declared:
\begin{verbatim}
void initmod(void (* odeparms)(int *, double *));
\end{verbatim}
That is, \emph{initmod} has a single argument, a pointer to a function
that has as arguments a pointer to an \texttt{int} and a pointer to a
\texttt{double}. In \FOR, the initialization routine has a single
argument, a subroutine declared to be external.  The name of the
initialization function is passed as an argument to the \deSolve
solver functions.

In \C, two approaches are available for making the values passed in
\emph{parms} visible to the model routines, while only the simpler
approach is available in \FOR.  The simpler requires that \emph{parms}
be a numeric vector.  In \C, the function passed from \deSolve to the
initialization function (called \emph{odeparms} in the example) copies
the values from the parameter vector to a static array declared
globally in the file where the model is defined.  In \FOR, the values
are copied into a COMMON block.

It is possible to pass more complicated structures to \C functions.
Here is an example, an initializer called \emph{deltamethrin} from a
...
\emph{\_RDy\_deltamethrin\_parms} and macro N\_PARMS are declared:
\begin{verbatim}
#define N_PARMS 63
...
\emph{R\_GetCCallable} which returns a function (called
\emph{get\_deSolve\_gparms} in this implementation) that returns the
parms argument as a SEXP data type.  In this example, \emph{parms} was
...
with argument \emph{jacres} rather than \emph{jacfunc} when an ODE.
...
\emph{lsodar} is an extended version of integrator \emph{lsoda} that
includes a root finding function. This function is spedified via
argument \emph{rootfunc}.
...
void myroot(int *neq, double *t, double *y, int *ng, double *gout,
            double *out, int *ip )
\end{verbatim}
where \emph{*neq} and \emph{*ng} are the number of state variables and
root functions respectively, \emph{*t} is the value of the independent
variable, \emph{y} points to a double precision array that contains
the current value of the state variables, and \emph{gout} points to an
array that will contain the values of the constraint function whose
root is sought.  \emph{*out} and \emph{*ip} are a double precision and

...
values. \code{\dots} (optional) are any other arguments passed to the
...
\end{verbatim}
  ...
  state variables y), and the next values are double precision values
  ...
  enough room is allocated for the output variables (\code{if (ip[0] <
...
\end{verbatim}
  ...
  Only for full Jacobian matrices, this is equal to \code{*neq}. In
  case the Jacobian is banded, \code{*nrowpd} will be equal to
  \code{*mu+*ml+1}.\footnote{Readers familiar with the implementation
    of the \proglang{FORTRAN} code \code{DVODE} may notice that this
    is not the format in which \code{DVODE} requires the specification
    of the Jacobian; this code needs an extra \code{mu} empty rows. As
    we have taken the philosophy to make the model specification
    independent of the integrator that will be used, the facility to
    use \code{vode} with a Jacobian specified in compiled code has
    been toggled off.  Use the related code \code{lsode} instead.}
\end{enumerate}
...
such objects to models defined in native code.
...
Note that in \proglang{R} names of \proglang{FORTRAN} DLLs functions
(here \code{func} and \code{jacfunc}) have to be given have to given
...
example(ccl4model) for a more comprehensive implementation)
...
\item \code{rule}, an integer describing how interpolation is to take
  ...
  calculation will stop if interpolation is necessary.  If it is

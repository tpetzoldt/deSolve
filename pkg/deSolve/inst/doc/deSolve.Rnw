...
\end{itemize}
For example, the next two statements both trigger the Euler method, the former
using the ``special'' code with a time step = 1, as imposed by the \code{times} argument, the latter
using the generalized method with a time step set by \code{hini}. Unsurprisingly, the former solution method
...
Several integrators (\code{lsoda}, \code{lsodar} and  \code{lsode}) 
can estimate a root.
...
that all values of eventtimes are now contained but only the subset of
times that have no exact or ``rather close'' neighbors in eventtimes.
...
$10^{-15}$\footnote{The accuracy of the machine can be determined with
...
but don't forget to think about this too. We leave this open as an exercise to the reader.

...
C W-7405-Eng-48.
C karline: changed INFO, to also pass the index of the variables
C          error scaling ~ index of variables            
...
C
      ...
      DIMENSION Y(*),YPRIME(*)
      DIMENSION INFO(25)        ! Karline: increased from 20 -> 25
      INTEGER NIND(3)           !          added  
...
C
     ...
     1  'daspk--  warning.. the last step terminated with a negative')
      call rprintfi1(
     2  'value of idid and no appropriate action was taken %i'
     & // char(0),idid)
      call rexit('- run terminated')
...
C KARLINE      
      IF(NIND(1) < NEQ)CALL SCALE(NEQ, NIND, RWORK(LWT), 1.d-1)  !H not known yet
...
C
      ...
      CALL DDAWTS(NEQ,INFO(2),RTOL,ATOL,Y,RWORK(LWT),RPAR,IPAR)
C KARLINE      
      IF(NIND(1) < NEQ) CALL SCALE(NEQ, NIND, RWORK(LWT), H0)
      CALL DINVWT(NEQ,RWORK(LWT),IER)
      IF (IER .NE. 0) GO TO 713
      GO TO 350
C
...
510   CALL DDAWTS(NEQ,INFO(2),RTOL,ATOL,RWORK(LPHI),RWORK(LWT),
     *            RPAR,IPAR)
      IF(NIND(1) < NEQ) CALL SCALE(NEQ, NIND, RWORK(LWT), H)
      CALL DINVWT(NEQ,RWORK(LWT),IER)
      IF (IER .NE. 0) THEN
        IDID = -3
        GO TO 527
...
C     reaching tout.
C! Karline toggled this off, version > 1.10.3 unless lots of printing requested
610   IF(IWORK(LKPRIN) .GE. 2) THEN   
      call rprintf(
     1  'daspk--  warning.. At current T (=R1) max number steps'
     &  // char(0))
      call rprintfd1(
     2  '      on this call before reaching tout %g' // char(0), TN)
      ENDIF
      GO TO 700
C
...
C
620   call rprintf(
     1  'daspk--  warning.. At T(=R1) too much accuracy requested'
     &  // char(0))
      call rprintf(
     2  '      for precision of machine. rtol and atol were' // char(0))
      call rprintfd1(
     3  '      increased to appropriate values %g'
     &  // char(0),  TN )
...
C
630   call rprintf(
     1  'daspk--  warning.. At T(=R1) some element of WT ' // char(0))
      call rprintfd1(
     2  '      has become less or equal than 0 %g' // char(0), TN )
      GO TO 700
C
...
650   call rprintf(
     ...
     &  // char(0))
      call rprintf(
     2  '      nonlinear solver failed to converge ' // char(0))
      call rprintfd2(
     3  '      repeatedly of with abs (H) = HMIN &g, %g'
     &  // char(0),  TN, H)
      GO TO 700
C
...
655   call rprintf(
     ...
     &  // char(0))
       call rprintfd2(
     2  '      preconditioner had repeated failures %g, %g'
     &  // char(0), TN, H )
      GO TO 700
C
...
660   call rprintf(
     ...
     &  // char(0))
      call rprintfd2(
     2  '      iteration matrix is singular %g, %g'
     &  // char(0), TN, H)
      GO TO 700
C
...
670   call rprintf(
     ...
     & // char(0))
      call rprintf(
     2  '      nonlinear solver could not converge ' // char(0))
      call rprintfd2(
     3  '      Also the error test failed repeatedly %g, %g'
     &  // char(0), TN, H )
      GO TO 700
C
...
675   call rprintf(
     ...
     &  // char(0))
      call rprintfd2(
     3  '      because ires was equal to -1 %g, %g'
     &  // char(0), TN, H)
      GO TO 700
C
...
C
680   call rprintf(
     1  'daspk--  warning.. At T(=R1) and stepsize H (=R2)' // char(0))
      call rprintfd2(
     2  '      ires was equal to -2 &g, %g' // char(0), TN, H )
      GO TO 700
C
...
685   call rprintf(
     ...
     &  // char(0))
      call rprintfd2(
     2  '      initial yprime could not be computed %g, %g'
     &  // char(0), TN, H0 )
      GO TO 700
C
...
690   call rprintf(
     1 'daspk--  warning.. At T(=R1) and stepsize H (=R2)' // char(0))
      call rprintfd2(
     2  '      IER was negative from psol %g, %g' // char(0), TN, H)
      GO TO 700
C
...
695   call rprintf(
     ...
     &  // char(0))
      call rprintfd2(
     2  '      linear system solver could not converge %g, %g'
     &  // char(0), TN,H )
      GO TO 700
C
C
...
C
701   call rprintfi1(
     1  'daspk-- element (= %i) of info vector is not valid'
     &  // char(0),ITEMP)
      GO TO 750
702   call rprintfi1(
     1  'daspk-- neq (= %i) < 0' // char(0), NEQ)
      GO TO 750
703   call rprintfi1(
     1  'daspk-- maxord (= %i) not in range' // char(0), MXORD)
      GO TO 750
704   call rprintfi2(
     1  'daspk-- rwork length needed, LENRW(= %i) exceeds LRW(= %i)'
     & // char(0), LENRW, LRW)
      GO TO 750
705   call rprintfi2(
     1  'daspk-- iwork length needed, LENIW(= %i) exceeds LIW(= %i)'
     &  // char(0), LENIW, LIW)
      GO TO 750
706   call rprintf(
     1  'daspk-- some element of rtol is < 0' // char(0))
      GO TO 750
707   call rprintf(
     1  'daspk-- some element of atol is < 0' // char(0))
      GO TO 750
708   call rprintf(
     1  'daspk-- all elements of rtol and atol are 0' // char(0))
      GO TO 750
709   call rprintfd2(
     1  'daspk-- INFO(4)=1 and TSTOP (= %g) behind TOUT (= %g)'
     &  // char(0),TSTOP,TOUT)
      GO TO 750
710   call rprintfd1(
     1  'daspk-- HMAX (= %g) < 0' // char(0), HMAX)
      GO TO 750
711   call rprintfd2(
     1  'daspk-- TOUT (= %g) behind T (= %g)' // char(0), TOUT, T)
      GO TO 750
712   call rprintf(
     1  'daspk-- INFO(8)=1 and H0=0' // char(0))
      GO TO 750
713   call rprintf(
     1  'daspk-- some element of WT <= 0 ' // char(0))
      GO TO 750
714   call rprintfd2(
     1  'daspk-- TOUT (= %g) too close to T (= %g)'
     &  // ' to start integration ' // char(0), TOUT, T)
      GO TO 750
715   call rprintfd2(
     1  'daspk-- INFO(4)=1 and TSTOP (= %g) behind T (= %g)'
     &  // char(0), TSTOP, T)
      GO TO 750
717   call rprintfi1(
     1  'daspk-- ML (= %i) illegal - either < 0 or > neq'
     &  // char(0),IWORK(LML))
      GO TO 750
718   call rprintfi1(
     1  'daspk-- MU (= %i) illegal - either < 0 or > neq'
     &  // char(0),IWORK(LMU))
      GO TO 750
719   call rprintfd2(
     1  'daspk-- TOUT (= %g) is equal to T (= %g)'
     &  // char(0),TOUT,T)
      GO TO 750
720   call rprintfi1(
     1  'daspk-- MAXL (= %i) illegal - either < 1 or > neq'
     &  // char(0), IWORK(LMAXL))
      GO TO 750
721   call rprintfi1(
     1  'daspk-- KMP (= %i) illegal - either < 1 or > MAXL'
     &  // char(0), IWORK(LKMP))
      GO TO 750
722   call rprintfi1(
     1  'daspk-- NRMAX (= %i) illegal -  < 0 '
     &  // char(0),IWORK(LNRMAX))
      GO TO 750
723   call rprintfd1(
     1  'daspk-- EPLI (= %g) illegal - either <= 0 or >= 1'
     &  // char(0),RWORK(LEPLI))
      GO TO 750
724   call rprintf(
...
726   call rprintf(
     ...
     &  // char(0))
      GO TO 750
727   call rprintfi1(
     1  'daspk-- Y(I) and IWORK(40+I) (I= %i) inconsistent'
     &  // char(0), IRET )
      GO TO 750
...
C
      ...
      IF (KPRIN .GE. 2) THEN
      call rprintfd1(
     1  'daspk--  in routine dlinsd--PNRM (= %g)' // char(0),  PNRM)
...
C-----------------------------------------------------------------------
 ...
 20         CONTINUE
            ...
            IF (KPRIN .GE. 2) THEN
      call rprintfid(
     1  'daspk--  constraint violation-PNRM (= %g), index =( %i)'
     &  // char(0),
     2   IVAR,PNRM)
              ENDIF
...
C
      ...
      call rprintfd1(
     1  'daspk--  min lambda (= %g)' // char(0), RLMIN)
...
C
      ...
      IF (KPRIN .GE. 2) THEN
      call rprintfd1(
     1  'daspk--  LAMBDA (= %g)' // char(0), RL)
      call rprintfd2(
     1  'daspk--  NORM(F1) = %g, NORM(F1NEW) = %g'
     &  // char(0), F1NRM, F1NRMP)
...
C-----------------------------------------------------------------------
 150  IRET = 0
      ...
      IF (KPRIN .GE. 1) THEN
      call rprintfd1(
     1  'daspk--  leaving routine dlinsd--FNRM (= %g)'
     &  // char(0),FNRM)
...
C-----------------------------------------------------------------------
 150  IRET = 0
      ...
      IF (KPRIN .GE. 1) THEN
      call rprintfd1(
     1  'daspk--  leaving routine dlinsk--FNRM %g' // char(0), FNRM)
...
C                1/(step size H).
C
C           WK = Real work array used by routine DATVPK and PSOL.
C
...
C                matrix HES.
C
C         NRE  = The number of calls to RES (i.e. DATVPK)
C
...
C-----------------------------------------------------------------------
C***ROUTINES CALLED
C   PSOL, DNRM2, DSCAL, DATVPK, DORTH, DHEQR, DCOPY, DHELS, DAXPY
C
...
C-----------------------------------------------------------------------
      ...
      DO 90 LL = 1,MAXL
        LGMR = LL
C-----------------------------------------------------------------------
C Call routine DATVPK to compute VNEW = ABAR*V(LL), where ABAR is
...
C-----------------------------------------------------------------------
        CALL DATVPK (NEQ, Y, TN, YPRIME, SAVR, V(1,LL), WGHT, Z,
...
C-----------------------------------------------------------------------
C Karline:
C rescaling of error term according to the index of each variable
C index 2 variables are scaled with 1/H
C index 3 variables are scaled with 1/H^2
C-----------------------------------------------------------------------

      SUBROUTINE SCALE(NEQ, NIND, SCAL, H)
      INTEGER NEQ, NIND(3) , I
      DOUBLE PRECISION SCAL(*), H
      IF(NIND(2).NE.0) THEN
             DO I=NIND(1)+1,NIND(1)+NIND(2)
                SCAL(I)=SCAL(I)/min(1.D0,H)
             END DO
      ENDIF
      
      IF(NIND(3).NE.0) THEN
            DO I=NIND(1)+NIND(2)+1,NIND(1)+NIND(2)+NIND(3)
                SCAL(I)=SCAL(I)/min(1.D0, H*H)
             END DO
      ENDIF
      RETURN
      END

      SUBROUTINE DATVPK(NEQ, Y, TN, YPRIME, SAVR, V, WGHT, YPTEM, RES,
...
C***DATE WRITTEN   890101   (YYMMDD)
C***REVISION DATE  900926   (YYMMDD)
CKS  30-06-2019: name changed to DATVPK 
C
C-----------------------------------------------------------------------
C***DESCRIPTION
C

...
to solve models that are written in \proglang{FORTRAN} or
...
integrators compatible (and as we think the omission of these vectors
in the \pkg{ODEPACK} solvers is a design flaw), we have altered the
...
\code{mymod.c} must first be compiled.
...
\item \code{DLLfunc} to test the implementation of the derivative function
  as used in ODEs. This function returns the derivative $\frac{dy}{dt}$.
...
returns what it should be.

\subsection{DLLfunc}
We test whether the ccl4 model returns the proper rates of changes.
...
described above (section 4.1), but where the production rate is included
...
Forcing functions are treated similar as parameters. This means:
\begin{itemize}
...
\code{approx}, howevers the dafault settings are often different.
...
\begin{itemize}
    ...
    \item \code{f } For method=\code{"constant"} a number between \code{0} and
    ...
    \item \code{ties } Handling of tied \code{times} values. Either a function
      ...
      NOT be investigated; this will mean that -if ties

...
\author{K. Soetaert\\
Centre for Estuarine and Marine Ecologoy\\
...
\pkg{deSolve}, the successor of \proglang{R} package \pkg{odesolve} is
a package to solve ordinary differential equations (ODE), differential
algebraic equations (DAE) and partial differential equations (PDE).
...
Soetaert et al. (subm.), where a formulation of two interacting
...
surprised to find the two vectors \code{yout} and \code{nout} at the
...
To run the models described aboove, the code in \code{mymod.f} and
...
This will create file \code{mymod.dll} resp. \code{mymodf.dll}. After loading the DLL, the
model can be run:
...
Also, if \code{func} is specified in compiled code, then \code{jacfun}
...
which may be an arbitrary R object.  In models defined in \proglang{R}
...
Not all integration routines included in \pkg{deSolve} can solve
\DLLmodels. To date those that can are:

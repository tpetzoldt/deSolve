...
\pkg{deSolve} \citep{deSolve}, the successor of \proglang{R}
package \pkg{odesolve} is a package to solve ordinary differential equations
(ODE), differential algebraic equations (DAE) and partial differential
equations (PDE).

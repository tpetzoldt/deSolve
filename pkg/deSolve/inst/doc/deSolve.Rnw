...
First all model variables are plotted versus time, and
finally Z versus X.
...
Most functions are based on original (FORTRAN) implementations, e.g.
the Backward Differentiation Formulae and Adams methods 
from ODEPACK \citep{Hindmarsh83}, or from \citep{Brown89,Petzold1983},
the implicit Runge-Kutta method RADAU \citep{Hairer2}, and a de novo 
implementation of several explicit 
Runge-Kutta methods \citep{Hairer1, Press92, Butcher1987}.

They can all be triggered from function \code{ode} (by
...
The default integration method, based on FORTRAN code LSODA is one that 
...
The explicit Runge-Kutta methods are de novo implementations in \proglang{C}, 
based on the Butcher tables \citep{Butcher1987}. 
They comprise simple Runge-Kutta formulae (Heun's method \code{rk2}, 
the classical 4th order Runge-Kutta, \code{rk4}) and several Runge-Kutta 
pairs of order 3(2) to order 8(7). 

The embedded, explicit methods are according to Fehlberg \citep{Fehlberg1967} 
(\code{rk..f}), Dormand and Prince \citep{Dormand1980,Dormand1981} 
(\code{rk..dp.}), Bogacki and Shampine \citep{Bogacki1989} 
((\code{rk23bs}), \code{ode23}) and Cash and Karp \citep{Cash1990} 
(\code{rk45ck}).
...
not optimally coded. The only well-implemented implicit Runge-Kutta is the 
(\code{radau}) method \citep{Hairer2} that will be discussed in the section
dealing with differential algebraic equations.

Only two formulae (\code{rk45dp7, rk45ck}) support dense output.
...
\code{A,b1,b2,c} are the coefficients of the Butcher table. 

It is also possible to define and use a new Runge-Kutta method:
...
It is simplest to do this with deSolve's S3-method \code{image}
...
found in \citet{Soetaert08}.  For 2-D problems, we refer to the
...
\begin{itemize}
\item \code{radau} which implicit Runge-Kutta RADAU5 \citep{Hairer2}
\item \code{daspk}, based on the backward differentiation code DASPK \citep{Brenan96}
\end{itemize}
...
@
\end{center}
\caption{Solution of the pendulum problem, an index 3 differential algebraic equation 
  using radau - see text for R-code}
\label{fig:pendulum}
\end{figure}

\clearpage
\section{Integrating complex numbers, function zvode}

Function \code{zvode} solves ODEs that are composed of complex variables.

We use \code{zvode} to solve the following system of
2 ODEs:
...
The solvers from \pkg{ODEPACK} can be optimised if it is known whether the
...
They can be specified as a data.frame, or in a function.  Events can
...
At time 1, 9, a value is added to variable v2, at time 1, state variable v2
is multiplied with 2, while at time 5, the value of v2 is replaced with 3.
...
\begin{figure}
\begin{center}
<<label=figevent1,fig=TRUE,echo=FALSE>>=
<<event1>>
@
\end{center}
...
This model describes the position (y1) and velocity (y2) of a bouncing
ball:
...
Then velocity (y2) is reversed and reduced with 10 percent.
...
2000 solving delay differential equations with dde23.
...
\begin{figure}
\begin{center}
<<label=figdde,fig=TRUE,echo=FALSE>>=
<<dde>>
@
\end{center}
...
The second sentence tells that, at this warning has been occurring 10 times, it will not be outputted again.

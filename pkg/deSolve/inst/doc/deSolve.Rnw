...
\begin{itemize}
\item as separate solvers \code{euler} and \code{rk4}. These implementations are 
  rather simple. The timestep used is determined by the time
  increment in the \code{times} argument.
\item as a special method of \code{ode}. In this case the time step used can be
  specified independently from the \code{times} argument, by setting argument
  \code{hini}. 
\end{itemize}  
For example, the next two statements both trigger the \code{euler} method, the former
using a time step = 1, as imposed by the \code{times} argument, the latter
using a time step set by \code{hini}. Unsurprisingly, the former solution method
...
There are S3 \code{plot} and \code{image} methods for plotting 0-D (plot), and
...
@
\end{center}
\caption{Plotting output and obs in one figure}
\label{fig:plotobs}
\end{figure}
...
Sometimes we want only output at a specific time value. We then use S3-method 
\code{subset} to extract 2-D variables at suitable time-values and use R's
...
values of the state variables at time = 50.

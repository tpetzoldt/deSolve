...
We implement the simple combustion model, which can be found on 
...
@
The different scenarios are plotted at once, and a suitable legend written.
...
legend("topright", lty = 1:2, col = 1:2, legend = c("p1", "p2"))
@
\setkeys{Gin}{width=\textwidth}
\begin{figure}
\begin{center}
...
If we do not select the variables to be plotted, then only the ones for which 
there are observed data are plotted.
Assume we have measured the total mass at the end of day 6:
<<>>=
...
The \code{hist} command plots the histogram for each variable; all plot 
...
The \code{image} command plots time-x images for models solved with 
...
and in concentric circles. This is a 1-D model, with cylindrical coordinates

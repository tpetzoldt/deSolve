...
c-----------------------------------------------------------------------
      ...
      subroutine crankres(t,y,yprime,cj,f,ierr,rpar,iparin)
      integer neqn,ierr,iparin(*), ipar(2)
      parameter(neqn = 24)
...
c NOTE: ipar elements are passed only from element 4 on
c the number of elements is in iparin(3) !
      if (iparin(3) .LT. 2) THEN
        call rexit("expecting at least 2 values in ipar")
      endif
